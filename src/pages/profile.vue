...
async function fetchProfile() {
    const res = await axios.get('http://localhost:5000/v1/api/user/profile', {
        headers: {
            'x-api-key':
    ...
    infoDetail.value = res.data.metadata.user
    levelInfo.value = res.data.metadata.level_detail
    progress.value = res.data.metadata.progress
}
...
<template>
    <div class="container mx-auto max-w-7xl mt-[72px]">
        ...
        <div class="p-10 flex flex-col items-center w-full">
            <div class="w-[800px] max-w-4xl">
                <div class="flex">
                    <div class="p-5 w-60 h-[240px]">
                        <img
                            class="rounded-full"
                    ...
                    <div
                        class="border-l-[1px] border-[#948979]/70 px-4 ml-5 py-5 w-[70%]"
                    >
                        <div class="mb-4">
                            ...
                            <div class="relative px-4">
                                <div
                                    class="w-full border-[1px] h-[20px] bg-white mt-2 rounded-2xl overflow-hidden"
                                >
                                    <div
                                        class="h-[20px] bg-gradient-to-r from-[#7eb8df] to-[#228fd8] transition-all"
                                        :style="{
                                            width: levelInfo.per * 100 + '%',
                                        }"
                                    ></div>
                                    <p
                        ...
                        <div class="py-2">
                            ...
                            >
                                Email:
                                <span class="relative ml-5 text-white"
                                    >{{ infoDetail.email
                                    }}<span
                                        v-if="infoDetail.status == 'pending'"
                                        class="absolute select-none left-[105%] top-0 w-20 rounded-full text-[10px] bg-red-400 text-black flex items-center justify-center font-bold"
                                        >*Chưa xác nhận</span
                                    ></span
                                >
                            </p>
        ...
        <div class="p-10 flex flex-col items-center w-full">
            <div class="w-full max-w-7xl">
                <div class="flex flex-wrap justify-center sm:justify-normal">
                    <div
                        v-for="course in progress.progress"
                        :key="course._id"
                        :class="
                            course.course != null
                                ? 'relative xl:w-1/3 sm:w-full md:w-1/2 p-4 flex flex-col rounded-2xl'
                                : 'hidden'
                        "
                    >
                        <router-link
        ...
        <div
            v-if="progress.achievements"
            class="p-10 flex flex-col items-center w-full"
        >
            <p
